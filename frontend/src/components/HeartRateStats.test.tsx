...
describe('HeartRateStats', () => {
  ...
  it('renders zero values when no data is provided', () => {
    render(<HeartRateStats data={[]} />);

    expect(screen.getByTestId('heart-rate-stats')).toBeInTheDocument();
    expect(screen.getByText('Heart Rate Statistics')).toBeInTheDocument();
    expect(screen.getByText('Minimum: 0 bpm')).toBeInTheDocument();
    expect(screen.getByText('Maximum: 0 bpm')).toBeInTheDocument();
    expect(screen.getByText('Average: 0 bpm')).toBeInTheDocument();
  });
